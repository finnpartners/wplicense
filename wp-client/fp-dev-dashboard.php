...
class FP_Dev_Dashboard {
    ...
    public function register_rest_routes() {
        register_rest_route( 'fp-dev/v1', '/status', [
            'methods'             => 'GET',
            'callback'            => [ $this, 'rest_status' ],
            'permission_callback' => [ $this, 'rest_status_permission' ],
        ]);
    }

    public function rest_status_permission( $request ) {
        $token = $request->get_header( 'X-License-Key' );
        if ( empty( $token ) ) return new WP_Error( 'unauthorized', 'Missing license key', [ 'status' => 401 ] );

        $settings = $this->get_settings();
        if ( empty( $settings['license_key'] ) || ! hash_equals( $settings['license_key'], $token ) ) {
            return new WP_Error( 'forbidden', 'Invalid license key', [ 'status' => 403 ] );
        }
        return true;
    }

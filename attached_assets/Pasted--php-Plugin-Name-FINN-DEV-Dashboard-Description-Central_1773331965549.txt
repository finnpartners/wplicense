...
class FP_Dev_Dashboard {
    ...
    // Updated to point to our future custom central API
    private $api_base_url    = 'https://licensing.finnpartners.com/wp-json/finn/v1';
    ...
    public function render_dashboard() {
        ...
        // Fetch available products from Custom API
    ...
    /* -------------------------------------------------------------------------
       LICENSING & HEARTBEAT LOGIC (CUSTOM API VALIDATION)
    ---------------------------------------------------------------------------- */
    ...
    public function check_for_updates( $transient ) {
        ...
        foreach ( $plugins as $file => $data ) {
            ...
            $url = "{$this->api_base_url}/update-check";

            // Append Product ID, License and Fingerprint for Authentication
            $url = add_query_arg([
    ...
    public function plugin_popup_info( $result, $action, $args ) {
        ...
        $res->sections = [
            'description' => 'Updates securely managed via FINN internal API.',
